"""Minimal hosted backend for the Streamhouse Twitch soundboard Extension.
...
<head>
  ...
  <title>{title} - Streamhouse Soundboard</title>
  <style>
...
<p>Streamhouse Soundboard lets Twitch viewers request sounds configured by a
broadcaster running Streamhouse Hub.</p>
...
<p>Pending requests expire after five minutes or are deleted as soon as Streamhouse Hub
...
Broadcasters can deactivate the Extension and disconnect Streamhouse Hub from the relay.</p>
<h2>Contact</h2>
...
TERMS_PAGE = _legal_page(
    ...
    f"""
<p class="updated">Effective July 21, 2026</p>
<p>By using Streamhouse Soundboard, you agree to these terms and Twitch's applicable
terms and policies.</p>
<h2>Using the Extension</h2>
<p>Streamhouse Soundboard sends a viewer's selected button request to the
broadcaster's Streamhouse Hub installation. The broadcaster chooses every available button,
...
class RelayState:
    ...
    def verify_hub(self, channel_id: str, supplied_key: str) -> None:
        expected = self.relay_keys.get(channel_id, "")
        if not expected or not hmac.compare_digest(expected, supplied_key):
            raise PermissionError("Invalid Streamhouse relay credentials.")
...
def main() -> None:
    ...
    print(f"Streamhouse soundboard relay listening on port {port}")
    try:
