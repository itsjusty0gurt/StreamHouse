param(
    ...
    [string]$RelayUrl,
    [string]$Output = ".\dist\SallySoundboardExtension.zip"
)

$ErrorActionPreference = "Stop"
$projectRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$source = Join-Path $projectRoot "twitch_extension"
$staging = Join-Path ([System.IO.Path]::GetTempPath()) ("sally-extension-" + [guid]::NewGuid())
...
try {
    ...
    Set-Content -LiteralPath $configPath -Encoding UTF8 -Value "window.SALLY_RELAY_BASE = `"$escapedUrl`";"
