...
from automation.core_tasks import PythonScriptTask
...
class PythonScriptTaskTests(unittest.TestCase):
    ...
    def test_runs_in_separate_python_and_supplies_trigger_context(self) -> None:
        ...
        script.write_text(
            "import json, os, sys\n"
            "print(json.dumps({'user': os.environ['SALLY_USER'], "
            "'context': json.loads(os.environ['SALLY_TRIGGER_CONTEXT']), "
            "'argument': sys.argv[1]}))\n",
            encoding="utf-8",
        )
    ...
    def test_background_mode_returns_after_starting(self) -> None:
        ...
        with patch("automation.core_tasks.subprocess.Popen") as popen:
