...
from ai.response_engine import ResponseDecisionEngine
...
class CompanionReasoningService:
    def __init__(
        self,
        ...
        if companion_settings is not None:
            self.settings = companion_settings
        else:
            try:
    ...
    def companion_settings(self, body: dict[str, Any]) -> dict[str, Any]:
        action = str(body.get("action", "get"))
        if action == "set":
            values = body.get("settings", {})
            if not isinstance(values, dict):
                raise ValueError("Companion settings must be an object.")
            self.settings = CompanionSettings.from_dict(values)
            self.settings_store.save(self.settings)
        elif action != "get":
            raise ValueError("Unknown Companion settings action.")
...
def create_server(
    host: str = "127.0.0.1",
    port: int = 8765,
    service: CompanionReasoningService | None = None,
) -> ThreadingHTTPServer:
    reasoning = service or CompanionReasoningService()

    class Handler(BaseHTTPRequestHandler):
        def do_POST(self) -> None:  # noqa: N802
            if self.headers.get("X-Sally-Protocol") != str(PROTOCOL_VERSION):
                self._send(409, {"error": "Unsupported Sally protocol version."})
                return
            try:
                ...
                if hasattr(reasoning, "companion_settings"):
                    routes["/v1/settings"] = reasoning.companion_settings
