...
$bundle = Join-Path $projectRoot "dist\StreamhouseAI"
$executable = Join-Path $bundle "StreamhouseAI.exe"
if (-not (Test-Path -LiteralPath $executable)) {
    throw "Missing build: $executable. Run tools\build\build_ai.ps1 first."
}

New-Item -ItemType Directory -Force -Path $releaseDirectory | Out-Null
$archive = Join-Path $releaseDirectory "StreamhouseAI-$version-windows-x64.zip"
...
Write-Host "Streamhouse AI archive: $archive"
