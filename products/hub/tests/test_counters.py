...
class CounterStoreTests(unittest.TestCase):
    ...
    def test_versions_invalid_payload_and_backup_recovery(self) -> None:
        ...
        (self.root / "index.json.bak").write_text('{"version": 1, "counters": []}', encoding="utf-8")
        (self.root / "old_counter.json").write_text('{"version": 1}', encoding="utf-8")
        self.assertEqual(self.service.list_counters(), ())
        quarantine = self.root.with_name("counters.pre-alpha-v1")
        self.assertFalse(self.root.exists())
        self.assertTrue((quarantine / "index.json").exists())
        self.assertTrue((quarantine / "old_counter.json").exists())
        self.service.create_counter(definition())
