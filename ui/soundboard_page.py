...
from config.twitch_extension import TWITCH_EXTENSION_CLIENT_ID
from soundboard.models import SoundboardButton, SoundboardPage
...
class SoundboardPageWidget(QWidget):
    ...
    def __init__(
        ...
        introduction = QLabel(
            ...
            "routine, so it can play audio and perform other Sally tasks."
        )
        ...
        self.relay_key_edit.setPlaceholderText("Private Sally relay key")
        self.relay_auto_connect_check = QCheckBox(
            "Connect automatically when Sally opens", hosted
        )
    ...
    def _relay_settings(self) -> tuple[SoundboardRelayConfig, str]:
        ...
        if not key:
            raise ValueError("Enter or generate a Sally relay key.")
        return config, key
